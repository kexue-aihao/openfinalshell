...
**界面目前只读**：能翻、能选中复制、`Ctrl+F` 查找、按编码重解。写回的 main 侧
（`sftp:fileSave`）已经接好并有 38 条用例覆盖，但**渲染进程还没有接线** ——
`Ctrl+S`、脏标记与三个确认框在片 3b。
...
保存这条路的 15 条护栏都对着改坏的实现验过会红（把闸门串起来、把基线摊开下发、
按 `text.length` 而不是编码后长度判上限、无视传进来的 charset/eol/hasBom……）。
其中最说明问题的一条：让 `fileView` 不记基线，38 条里红 29 条 ——
