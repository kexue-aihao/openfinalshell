...
冲突检测、截断闸门全在 `remoteTextWrite.ts`。那一份当初是为了与外部编辑器共用才提出去的
（两份实现漂开的第一天就会有一边写坏文件）；那条路已经删掉，但分层留着 ——
语义与编排分开，语义那半有自己的用例。保存这一层只做编排的四件事：
...
**① 正文不进 store，也不进 React 状态。** doc 最大 8MB，每敲一个键复制一份字符串再
...
从 2MB 放宽而来，量了四条约束才定的。**绑住它的是渲染进程的内存，不是 CPU 也不是带宽。**

| 约束 | 结论 |
|---|---|
| main 侧那几道 O(n) 关卡（decode / lossless 往返 / encodeFidelity / sha256 / looksBinary） | 不是瓶颈：8MB 打开 96ms、保存 92ms；32MB 也在 0.4s 以内 |
| 每次按键的代价 | **曾经是瓶颈，已修** —— 见下 |
| 渲染进程内存 | **天花板在这里**：8MB 的中文文件常驻约 37MB（UTF-16 字符串 + CM 的 rope + store 那份），×10 个打开上限 = 370MB。16MB 那档就是 740MB |
| 传输耗时 | 2MB 实测写 1.7s / 读 2.5s；8MB **线性外推**约 5–8s。⚠️ 这一条是推的不是量的 |

**每次按键的代价这条值得单独说，因为它是我自己在片 3b 引进的。** 脏标记原先每敲一个键做一次
`doc.toString()` 比较 —— O(整个文档)，实测每按键 1MB 1.8ms / 8MB 7.4ms / 32MB **14.3ms**
（一帧才 16ms）。在 2MB 上察觉不到，所以它一直藏着；正是"要放宽上限"这件事把它翻出来的。
改成"先比 `length`（O(1)）→ 长度相同才 `Text.eq`（rope 之间比，内容不同时在第一个不同的块上短路）"
之后是 0.001ms。这是放宽上限的**前置条件**，有护栏盯着它别退回去
（两种写法行为完全一样、所有行为用例对两者都绿，差别只在代价 —— 只能查源码）。

顺带这一片抓到一条**静默停止测试**的用例：那条"按编码后的长度判上限"的断言写死了
80 万个中文字符（当时 = 2.4MB > 2MB 上限）。上限放宽到 8MB 之后 2.4MB 不再越界，
它照样绿、只是什么都没验了。现在条数从 `MAX_EDIT_BYTES` 推出来，上限再动也不会失效。
`benchEditIo` 的档位同理改成自动跟着上限走。
...
- **编辑远端文件的几条边界**（都是有意为之，不是没做完）：
  ...
  - **软链会被解析成真身再编辑**（`/etc/nginx/sites-enabled/*` 全是软链），
    所以编辑完那条软链还是软链。编辑期间有人把软链重新指向别处的话，**保存会停下来报错**
    而不是打到旧真身上 —— 那条路径此刻通向的是另一个文件，覆盖它从来不在你授权的范围里。
  - 单文件上限 8MB、同时最多打开 10 个；**含 NUL 字节的文件一律拒绝**（含 UTF-16 文本 ——
    往返一次极易损坏，宁可让你走下载-改-上传）。
  - **当前编码解不干净的文件是只读的**（状态条上有"编码可能不对"的提示）。里面有这个编码里
    不合法的字节，你看到的内容已经有字符被替换过 —— 存回去会把那些你从没看见的字节永久改写。
    换个编码重开、确认内容正常再编辑。
  - **打的字存不下去时会拒绝保存并点名那个字符**（在 GBK 文件里粘一个 emoji 就是这种情况）。
    iconv 对表示不了的字符不报错、悄悄换成 `?`，所以这一条必须在写之前拦住。
  - **混用行尾的文件，保存会把整个文件的行尾统一掉**（统一成状态条上显示的那一种），
    所以那种文件的往返不可能逐字节相同。状态条上会先亮一个"行尾混用"。
  - 服务器不通告 `posix-rename@openssh.com` 时会明确拦下来问你，退化路径是
    "原文件改名备份 → 新内容改名就位 → 删备份"；断在中间的话原内容以 `.ofsbak-` 开头留在同目录。
  - **v0.2 起没有"用外部编辑器打开"了**：那条路（下载到 `%TEMP%` + 起一个 exe + 盯文件存盘）
    整条删掉，改由内置编辑器承担。这不只是"少一个功能"——
    远端文件的明文副本不再落到本机磁盘、`%TEMP%` 不再需要清扫、子进程面从两处收窄到一处、
    设置里那个"编辑器 exe 路径"（连同它导入配置时的提权链）一起消失。
    顺带那条老路最大的盲区也没了：**编辑器"另存为"到别的路径时什么都不会上传、也不会报错**
    —— 内容一直在编辑器缓冲区里，就没有"存到别处"这回事。
    `settings.sftp.doubleClickAction` 的 `'open'` 语义搬到内置编辑器上，用户的选择不用改。
- **快速删除（`rm -rf`）的几条边界**：
