...
能翻、能选中复制、`Ctrl+F` 查找、按编码重解，**能改、`Ctrl+S` 存回远端**。
...
**输入法已经验到了**（片 3b 补的，片 2 那会儿确实验不了 —— 只读不接受输入就没有组词过程）：
冒烟 step 8.9 用 CDP 的 `Input.imeSetComposition` 造出真实的 composition 序列
（`ni` → `nihao` → 提交「你好」），断言三件事：内容进了文档、**没有拼音残留**、
只插了一遍。CodeMirror 对组词是特殊处理的（组词期间暂停自己的 DOM 读回），
接错的症状就是文档里躺着 `nihao你好`。实测通过。

#### 保存（`sftp:fileSave`）的两条分界

写回的语义一条都不在保存这一层 —— 原子替换、临时文件排他创建、非原子替换的备份顺序、
冲突检测、截断闸门全在 `remoteTextWrite.ts`，与外部编辑器那条路共用同一份实现
（两份实现漂开的第一天就会有一边写坏文件）。保存这一层只做四件那条路不需要的事：
按编码/行尾/BOM 把字符串变回字节、取基线、四道硬拒、并发拒绝。

**第一条分界：闸门 vs 硬拒。** 写错方向就是数据损失。

| 闸门（回一个结果分支，界面弹确认框，用户点了带 gate 再调一次） | 硬拒（抛异常，**没有任何 gate 能越过**） |
|---|---|
| 远端在你编辑期间被改过 | 用户打的字在目标编码里存不下去（`?` 换不回来） |
| 服务器不支持原子替换 | 打开时那份字节本来就解不干净（会毁掉他从没看见的字节） |
| 内容缩水得像手滑 | 编码后超过 `MAX_EDIT_BYTES` |
| | 基线不在了 / 这条路径此刻指向的是另一个文件 |

判据是一句话：**一次确认能不能让它变正确**。用户点"仍然保存"并不能让一个 emoji
变成 GBK 里存在的字，也不能让被替换字符顶掉的字节回来。

三个闸门是三个**必填** boolean（`RemoteSaveGates`）而不是一个 `force`：老路把三件事
挤成一个开关，于是用户点"仍然覆盖"（我接受远端改动被盖掉）顺带把非原子替换也放行了 ——
他同意的是前者，承担的是后者。`test/unit/fileSave.test.ts` 里有两条专门守这个，
谁哪天"简化"回一个开关就红。

**第二条分界：基线不下发。** 冲突检测的基线（sha / size / mtime）只存在 main 侧的
`editBaselines.ts` 里，一个字节都不给渲染进程。一旦它能被回传，渲染进程的任何 bug ——
一次错误的浅拷贝、一次"重试时顺手用当前值填一下" —— 都能构造出"基线正好等于远端现状"，
于是检测永远说"没变过"。这类 bug **没有任何症状**：它只在别人同时改了同一个文件时
才造成损失，而那正是没人在测试里覆盖的那一天。代价是 `sftp:fileView` 不再是纯函数
（它要记一条内存基线），这一点写在那个函数的注释里，没有含糊过去。

注册表有 200 条上限、满了淘汰最老的一条 —— 这**是安全的**，因为基线不在 = 硬拒并让
用户重新打开，绝不会退化成"没有基线就当没变过、直接写"。有一条用例专门守它。

保存这条路的护栏都对着改坏的实现验过会红：main 侧 15 条（把闸门串起来、把基线摊开下发、
按 `text.length` 而不是编码后长度判上限、无视传进来的 charset/eol/hasBom……），
渲染进程侧 21 条。其中最说明问题的一条：让 `fileView` 不记基线，38 条里红 29 条 ——
这就是**用真的 `fileView` 铺基线而不是手搓一个**买到的东西，手搓的话那条接线断了照样全绿。

#### 界面这一侧：三个容易出事的地方

**① 正文不进 store，也不进 React 状态。** doc 最大 2MB，每敲一个键复制一份字符串再
重渲染整棵子树是可感知的卡。所以 store 里存的是"上一次读到／存成的那份"，
当前内容只在 CodeMirror 自己的 state 里，保存时通过一个 getter 取。脏与否由编辑器
比出来告一声 —— 而 `setDirty` 在值没变时**返回同一个数组**，否则每次按键还是会重渲染。

**② 每个文件各自暂存 EditorState，这是正确性要求不是优化。** 只有一个 EditorView，
换标签时如果无条件按新内容重建，"改了 A → 切到 B → 切回 A"就会把 A 的未保存改动
静默扔掉。这段判断被提成纯函数 `planDraftSwap`（三种结论：留住 / 重建 / 恢复暂存），
因为它在 node 里跑的测试环境下没有 DOM、没法直接测 CodeMirror。顺带买到的是撤销历史
与选区跟着文件走。暂存必须按打开着的 key 清理 —— 不清的话"关标签放弃改动"之后
再打开，草稿会诡异地复活。

**③ `history()` 此前压根没装。** 只读时没人按 Ctrl+Z 所以一直没暴露
（`historyKeymap` 不配 `history()` 是空转，而 `defaultKeymap` 里并不含撤销）。
可写之后这是不可接受的，已补。

冒烟 step 8.9 在真产物里把整条闸门循环跑了一遍，而这是 fixture 服务器白送的：
它不通告 `posix-rename`，所以每次保存都必然先撞上 nonAtomic 那道闸门。实测结论：
Ctrl+S 触发保存 → 被拦下 → **取消之后远端一个字节没变、脏标记还在** → 再来一次并确认
→ 远端读回来含刚打的中文。然后从背后改一次远端，再存 —— **conflict 与 nonAtomic
两个框连着弹**，这正是三个开关拆开的真产物红证：老那种一个 `force` 管三件事的设计
在这里只会弹一次，用户确认的是"覆盖别人的改动"，承担的却还有"文件会短暂不存在"。

两处刻意的取舍，别当成漏掉的：
- **`Ctrl+S` 只在编辑器有焦点时生效**，不是全局快捷键 —— 终端里 Ctrl+S 是流控（XOFF），
  抢掉它会让 `less` / `vim` 用户莫名其妙。有一条护栏盯着它别变成全局。
- **`Tab` 在编辑器里插缩进，不再移出焦点**（`indentWithTab`）。这是键盘可达性的折衷：
  在代码编辑器里按 Tab 却跳走更糟，而这一格有别的出口（鼠标、Ctrl+Tab 切会话标签）。
