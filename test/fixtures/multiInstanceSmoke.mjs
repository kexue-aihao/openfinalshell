...
// Opt in without seeding Chromium state: both first processes must still bootstrap one Vault key.
mkdirSync(join(data, 'config'))
writeFileSync(join(data, 'config', 'settings.json'), JSON.stringify({ multiInstanceEnabled: true }))
...
try {
  ...
  // Both processes start without Chromium state: they must share one protected encryption key.
  ...
  await a.invoke('settings:set', { multiInstanceEnabled: false })
  assert.equal((await b.invoke('app:instanceInfo')).canOpenNewWindow, false)
  assert.equal((await a.invoke('app:instanceInfo')).canOpenNewWindow, false)
  await assert.rejects(b.invoke('app:newWindow'))
  const blocked = launch(['--new-instance'])
  await until(() => blocked.exitCode !== null)
  assert.equal(blocked.exitCode, 0)
  assert.equal(procB.exitCode, null)
  passed('disabling multi-instance blocks IPC and command-line launches without closing existing sessions')
  ...
  await b.invoke('settings:set', { multiInstanceEnabled: true })
  assert.equal((await b.invoke('app:instanceInfo')).canOpenNewWindow, true)
  ...
  assert.equal((await r.invoke('settings:get')).multiInstanceEnabled, true)
  assert.equal((await r.invoke('app:instanceInfo')).canOpenNewWindow, true)
  ...
  await r.invoke('settings:set', { multiInstanceEnabled: false })
  ...
  const disabledPort = await port()
  const disabledRestart = launch([`--remote-debugging-port=${disabledPort}`])
  const disabledClient = await connect(disabledPort)
  assert.equal((await disabledClient.invoke('settings:get')).multiInstanceEnabled, false)
  assert.equal((await disabledClient.invoke('app:instanceInfo')).canOpenNewWindow, false)
  await disabledClient.evaluate('window.close()').catch(() => {})
  await until(() => disabledRestart.exitCode !== null)
  passed('both enabled and disabled multi-instance preferences survive a full restart')
