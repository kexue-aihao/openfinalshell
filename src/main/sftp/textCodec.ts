...
export interface DecodeResult {
   ...
   * 所以调用方必须能看见它、在保存前告诉用户一句（界面上那个"行尾混用"标签）。
   * （不做"保留混用"的方案：那要求编辑器逐行记住原始行尾，
 ...
 * （曾经 editGuards 里有一份同样的实现，服务于"存盘前后比对行尾有没有被翻面"。
 *   那个函数随外部编辑器一起删掉了，所以现在这是全项目唯一一份。）
 */
